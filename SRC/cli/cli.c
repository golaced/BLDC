...
#define verHexNum 57
uint8_t verHex[verHexNum] = {0x76,0x30,0x2E,0x39,0x30,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x4F,0x6C,0x6C,0x69,0x57,0x20,0x33,0x32,0x33,0x42,0x47,0x43,0x00,0x00,0x00,0x00,0x76,0x31,0x2E,0x33,0x30,0x20,0x46,0x31,0x30,0x33,0x52,0x43,0x00,0x00,0x00,0x00,0x5A,0x00,0x59,0x00,0x03,0xFF,0xE7,0x2A,0x6F};
...
float readFloatCLI(void)
{
    ...
    data[index] = '\0';
    printUSART("readFloatCLI data=%s\r\n",data);
...
void readCliPID(unsigned char PIDid)
{
    ...
    pid->P              = readFloatCLI()/100;
    pid->I              = readFloatCLI()/1000;
    pid->D              = readFloatCLI()/10000;
    readFloatCLI();
    readFloatCLI();
    pid->iTerm          = 0.0f;
    ...
    readFloatCLI();
    printUSART("readCliPID P=%4.4f,I=%4.4f,D=%4.4f\r\n",pid->P,pid->I,pid->D);
/*
    pid->B              = readFloatCLI();
    pid->P              = readFloatCLI();
    pid->I              = readFloatCLI();
    ...
    switch (cliQuery)
    {
        ...
        case 'a': // Rate PIDs
            cliPrintF("\nRoll Rate PID:  %9.4f, %9.4f, %9.4f, %9.4f, %9.4f, %s\n", eepromConfig.PID[ROLL_PID].B,
                                                                                   ...
                                                                                   eepromConfig.PID[ROLL_PID].dErrorCalc ? "Err" : "State");
            ...
            cliPrintF("Pitch Rate PID: %9.4f, %9.4f, %9.4f, %9.4f, %9.4f, %s\n",   eepromConfig.PID[PITCH_PID].B,
                                                                                   ...
                                                                                   eepromConfig.PID[PITCH_PID].dErrorCalc ? "Err" : "State");
            ...
            cliPrintF("Yaw Rate PID:   %9.4f, %9.4f, %9.4f, %9.4f, %9.4f, %s\n",   eepromConfig.PID[YAW_PID].B,
                                                                                   ...
                                                                                   eepromConfig.PID[YAW_PID].dErrorCalc ? "Err" : "State");
            cliQuery = 'x';
            cliPrintF("o");
            ...
            printUSART("Pitch Rate PID: %9.4f, %9.4f, %9.4f, %9.4f, %9.4f, %s\n",   eepromConfig.PID[PITCH_PID].B,
                                                                                   eepromConfig.PID[PITCH_PID].P,
                                                                                   eepromConfig.PID[PITCH_PID].I,
                                                                                   eepromConfig.PID[PITCH_PID].D,
                                                                                   eepromConfig.PID[PITCH_PID].windupGuard,
            ...
            printUSART("Yaw Rate PID:   %9.4f, %9.4f, %9.4f, %9.4f, %9.4f, %s\n",   eepromConfig.PID[YAW_PID].B,
                                                                                   eepromConfig.PID[YAW_PID].P,
                                                                                   eepromConfig.PID[YAW_PID].I,
                                                                                   eepromConfig.PID[YAW_PID].D,
                                                                                   eepromConfig.PID[YAW_PID].windupGuard,
        ...
        case 'A': // Read Roll PID Values
            printUSART("Roll Rate PID Received....\r\n");
            readCliPID(ROLL_PID);
        ...
        case 'B': // Read Pitch PID Values
            printUSART("Pitch Rate PID Received....\r\n");
        ...
        case 'C': // Read Yaw PID Values
            printUSART("Yaw Rate PID Received....\r\n");
            readCliPID(YAW_PID);
