...
void setPWMFastTable(int *pwm, float angle, float power)// power  50 is default
{
    ...
    angleInt = angleInt % SINARRAYSIZE;//1024的取整
...
void pwmMotorDriverInit(void) 
{
	...
	//tim2 没有重映像  GPIO_PinRemapConfig(GPIO_PartialRemap2_TIM2,ENABLE);
    //tim3 没有重映像  GPIO_PinRemapConfig(GPIO_PartialRemap_TIM3,ENABLE);
    //tim4 没有重映像  GPIO_PinRemapConfig(GPIO_Remap_TIM4,ENABLE);
